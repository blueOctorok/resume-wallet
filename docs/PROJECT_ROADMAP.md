...
| Employer Composable Hub Phase 3 — Full Gating | ✅ Done | ALL candidate outreach gated behind employer blocks (`requiredEmployerBlocks` on `BlockDefinition`). True 1:1 mapping: `employer-portfolio-requests` → Portfolio, `employer-dot-screening` → DOT App, `employer-mvr-orders` → MVR, `employer-psp-mvr-bundle` → PSP + MVR. Resume removed from outreach (core block, always installed). Invite flow creates `candidate_requests` for screening blocks so FCRA disclosure fires. Migrations `076`–`077`. **Combined disclosure + order form:** employer-initiated MVR/PSP renders the disclosure form inline (not modal) with SSN field and auto-submits the Accio order after signing — candidate never sees the USDC self-order form. New endpoint `POST /api/candidate/fulfill-screening`. |
