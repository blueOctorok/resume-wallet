...
| **Stormi hub welcome (AI step 1 + replay)** | ✅ Done | `walkthrough-ai.ts` + `candidateHubStaticSteps`; **My Hub → Stormi Journey Guide** → `requestWalkthrough()` remount + refetch; nav no longer opens legacy `StormiJourneyGuide` drawer for that action; mobile **Career path** FAB scrolls to sticky sidebar (`CHANGES.md`) |
