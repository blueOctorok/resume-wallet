...
| Employer Composable Hub Phase 3 — Full Gating | ✅ Done | ALL candidate outreach gated behind employer blocks (`requiredEmployerBlocks` on `BlockDefinition`). True 1:1 mapping: `employer-resume-requests` → Resume, `employer-portfolio-requests` → Portfolio, `employer-dot-screening` → DOT App, `employer-mvr-orders` → MVR, `employer-psp-mvr-bundle` → MVR + PSP. `CandidateOutreach` block picker + `CareerCardModal` request buttons filter by installed employer blocks. Migrations `076` (seed) + `077` (split talent-outreach into resume/portfolio). |
