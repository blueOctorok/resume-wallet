...
> **Candidate EV (2026-09-09):** Packet is 4-part driver authorization + 5-part Safety Performance History, transcribed from the boss's PDF (`CDL_Driver_DOT_Employment_Verification_Workflow_...pdf` → `docs/AUTH_FORM.md`). No drug/alcohol question block — page 2 Part 4 is "Other Safety Performance Information," Part 5 is "Records Holder Certification." Auth Part 2 can be submitted without contact info (employer-side research). DKIM still gates “verified.” Apply `109_evr_driver_send_declined.sql`.

> **EV Consent Stack (2026-09-22):** The EV block now enforces the three Track B legal artifacts (`docs/EV_CONSENT_STACK.md`, canonical text in `src/lib/ev-consent-documents.ts`). Outbound send hard-gates on a driver disclosure+authorization artifact (`ev_authorizations`, 403 without it); employers request a share via a per-request clickwrap (`ev_share_requests`, pending only, candidate-initiated application context only — never talent-search browse); EV content unlocks **solely** through the driver's Step 6 acknowledgment (`ev_share_grants`, proof-summary payload, forward-only revoke, `ev_access_log` audit). Strict visibility shipped with it: no itemized EV or verified-count on cards, talent search, previews, or public token links — only a neutral "Employment verification available on request" line. Global share/hold buttons removed (`driver_share_consent` is historical). Apply `110_ev_consent_stack.sql`. **Open flag:** `previous_employer_verified` attestation fact still bypasses the share gate — needs a Midnight-pipeline decision.

> **MVR accident parity (2026-09-15) — ✅ closed:** MVRs reported zero accidents for every driver ever pulled — Accio sends accidents as `<mvr_violation>` with `violation_type` `DRIVER ACCIDENT` / `ACCD`, and the parser looked for an `<mvr_accident>` tag that does not exist. Fixed in `accio-xml-parser.ts` and backfilled (180 orders / 226 records). Follow-ups all shipped the same day: accident count now renders on `MvrSection` (shared by the candidate hub *and* the employer talent modal), on the employer company-paid MVR block, and on the legacy `CareerCard`, with the four `mvr_results` selects that never read `accident_count` fixed; the rollup shape is declared once as `MvrResultsSummary` so it can't drift again; and `classifyMvrViolationCategory` now logs unrecognized `violation_type` values (0 false positives across all 1,084 stored payloads). **Most consequential find:** `mvr_clean_36_months` gated only on violations, so one live attestation was Midnight-proven as "Clean MVR" over a `PROPERTY DAMAGE` accident inside its own window — the predicate now gates on accidents and that attestation was superseded. See `CHANGES.md` ("MVRs never reported accidents" + follow-on).

> **⚠ Action required — MVR accident fix is not deployed (2026-09-15).** The accident work is uncommitted, so the Accio webhook still hits deployed code with the old parser: order `2eab4216-…` completed at 13:27 UTC *after* the backfill and was written with `accident_count = 0`, accident misfiled under `additionalDriverInfo`. Repaired, but it will keep happening per-order until deploy. **After deploying, re-run `scripts/backfill-mvr-accidents.ts --apply` then `scripts/supersede-stale-clean-mvr-attestations.ts --apply`** to sweep the gap. Both scripts are idempotent and dry-run by default.

> **Open: adverse license actions filed as informational (2026-09-15).** `DRIVER REVOCATION`, `DISQUALIFICATION`, `DRIVER WITHDRAWAL`, and `SURRENDER` (~50 records) classify as Additional Driver Info. They read like adverse license actions that belong with suspensions on a DQ file. Reclassifying moves employer-visible suspension counts and may invalidate existing attestations, so this needs a product decision — not a parser tweak. Same failure mode as the accident bug, one bucket over.
...
| **Phase 6 — Agency dashboard** | 🚧 **In progress** | **First slice shipped (2026-08-04):** Employer hub **Drivers — DQ monitor** (roster by name → person detail → DQ checklist). Shared registry + company/driver lenses (`dq-file-registry.ts` / `dq-file-status.ts`). Driver hub **Your DQ progress** mirror. Live status for MVR, PSP, DOT, CDLIS consent, employment verification; placeholders for DL photos, med card, criminal BG, drug screen, Clearinghouse, CDLIS inquiry. **Still todo:** multi-company share links for carriers, hire-date 30-day EV clock UI wiring, Key product orders, document upload blocks (Phase 2). Never Pace-special-cased — any company with screening blocks gets the monitor. |
...
Full layout for non-engineers: `docs/provven-access-model.pdf` (shareable, generated from `docs/provven-access-model.html`) and canvas `provven-access-model.canvas.tsx` (Cursor workspace canvases).
...
| **Pingram SMS Outreach Text** | **✅ Done (code)** · **A2P pages ready** | **Invite SMS via Pingram. Public `/privacy` + `/terms` with carrier SMS language + landing footer links (2026-09-17). Ops: deploy, then Pingram files Provven A2P. Still to do after approval: recruiter verbal-consent attestation + confirmation SMS. Migration `103`.** |
...
**2026-08-31:** Candidate hub block is live. It scans DOT Form 3 (and resumes) for jobs + hiring-manager contact so the driver does not retype them. Employer-orchestrated DOT outreach (`employer-employment-verification`) is still coming soon.
