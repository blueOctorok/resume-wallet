...
| Employer Composable Hub Phase 3 — Full Gating | ✅ Done | ALL candidate outreach gated behind employer blocks (`requiredEmployerBlocks` on `BlockDefinition`). New blocks: `employer-talent-outreach`, `employer-dot-screening`. `CandidateOutreach` block picker + `CareerCardModal` request buttons filter by installed employer blocks. Migration `076` seeds Pace Drivers. |
