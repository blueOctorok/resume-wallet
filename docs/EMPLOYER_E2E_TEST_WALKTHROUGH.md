...
Quick guide to test employer flows: Talent Search → Career Card → Requests/Recruit → Pipeline (Kanban) → Notes.
...
2. **At least one active job posting** – In Employer Hub, create a job (Driver or Developer) and keep it active.
3. **Candidate data (optional but useful)** – One or more users with **Driver** or **Developer** role who have built their hub (profile, resume, DOT app, etc.) so they appear in talent search and have a meaningful career card.
...
| 2 | Click Find Talent → Search (e.g. Driver) | Candidate list appears |
| 3 | Click a candidate | Career card modal opens (no z-index/scroll issues) |
| 4 | On career card: Order MVR or Request document | Request created; candidate notified (email + hub) |
| 5 | On career card: Recruit Candidate → pick job → Send | Application created; candidate in pipeline |
| 6 | In hub: drag applicant to “Reviewing” | Status updates; candidate email (if Resend on) |
| 7 | Open applicant → add note + rating in right panel | Saved; visible in timeline |
| 8 | Log in as that driver/dev → open hub | “Requests” shows employer request; can complete/decline |
...
- **Recruit fails / no jobs:** Create at least one **active** job posting for your company.
