...
#### T1.8-admin — Admin route auth (email allowlist) · ✅ Done · pending user commit · 2026-05-30
Admin gating moved off wallet entirely. `requireAdmin` (`src/lib/admin-auth.ts`) is now **async**, resolves the **Supabase session** (`createServerSupabase().auth.getUser()`), and checks `isAdminEmail(email)` against **`ADMIN_EMAILS`** (replaces `ADMIN_WALLETS` in env). Returns `{ authorized, userId, email, error }`. All **33** `requireAdmin(request)` callers gained `await`; audit/log `auth.walletAddress` → `auth.email`. The 5 routes with their own inline `ADMIN_WALLETS` const + `isAdmin(wallet)` (`employer-requests` [+`/[id]`], `jobs`, `companies/[id]/members` [+`/[memberId]`]) now use `requireAdmin`. Audit `actorUserId` / `reviewed_by` / `approved_by` use `auth.userId` directly (session id = `users.id`, no wallet→id lookup). Per-row admin badge/warning use `isAdminEmail(email)`. `AdminDashboardShell` shows the admin's email. Build green · IDE lint clean · `rg "ADMIN_WALLETS|isAdminWallet"` = 0 in code.

**Why this was the right time:** the T1.12c cutover made new email signups carry an `auth:<uuid>` placeholder that could never be in a wallet allowlist → no new admin could be added. The two working admins only squeaked through on their migrated DB wallet; the third (metro) was deleted as an orphan in migration 093.

**Follow-up (functional no-op):** strip the 41 now-ignored `x-wallet-address` sends across the 18 admin UI files (server uses the session cookie). **Prod action:** set `ADMIN_EMAILS` in Vercel env.
...
|                        |                                     |
| ---------------------- | ----------------------------------- |
| Status                 | 🟡 In progress — **T1.12b + T1.12c + T1.12.1 done**; full SDK/provider removal (T1.12d) deferred |
...
- **T1.12b — Drop client wallet dependence.** ✅ **Done · ff8e396 · 2026-05-30.** Removed vestigial `x-wallet-address` client sends from ~56 files where the target API route resolves identity via `getStormUserIdFromRequest` (session cookie). **Kept** headers on the explicit allowlist: `admin/**`, `/api/storm/history`, create-on-write routes (`/api/resumes/create`, `/api/resumes/upload`, GET `/api/resumes`, `/api/driver-applications/save-progress`, `/api/user/profile-setup`), and STORMI-unlimited routes (`/api/ai/chat`, `cover-letter`, `parse-resume`, `interview-prep-quiz`, `job-talking-points`, `/api/jobs/recommended`). Did **not** strip `walletAddress` from stores/props (T1.13). `npm run build` green.
...
- `rg "x-wallet-address" src/` now hits **admin + allowlisted legacy routes only** (storm/history, create-on-write resume/DOT/profile-setup, STORMI-unlimited AI routes, ava-chat/walkthrough-ai) after **T1.12b**. ✅
...
|                        |                                                                                     |
| ---------------------- | ----------------------------------------------------------------------------------- |
| Status                 | ✅ **Done · 2026-05-30 (migrations 093+094 applied via dashboard).** FK `users_id_fkey` exists + `convalidated=true` (`FOREIGN KEY (id) REFERENCES auth.users(id) ON DELETE CASCADE`); 153 users, **0 orphans**. Pace Drivers owner + 3 members intact. Prevention fix (`getOrCreateUserByWallet` resolves `auth:<uuid>` by id) shipped in code. |
...
>
> **Done 2026-05-30:** (1) ✅ prevention fix (`user-by-wallet.ts`) → (2) ✅ migrations 093+094 authored → (3) ✅ ran 093 (after fixing a `career_cards`-is-a-view error — removed that DELETE; the view derives from base tables) → (4) ✅ gate = 0 → (5) ✅ ran 094. FK validated; 153 users, 0 orphans; Pace intact.
...
|                        |                                            |
| ---------------------- | ------------------------------------------ |
| Status                 | 🟡 **Partial — orphan personal wallet UI deleted 2026-05-30.** `walletAddress` store field + company-wallet stack still pending (T1.12d). Teardown map: `docs/midnight/T1_13_WALLET_TEARDOWN_MAP.md`. |
