...
- **NOT a "Web3 hiring app"** in the dated, crypto-bro sense — no feed, no speculation, no wallet UX. But the cryptography is **not** hidden either: Provven celebrates Midnight / zero-knowledge / selective disclosure as its public credibility narrative (`provven.com`, DEC-2026-05-016), and the chain is **load-bearing** for network-independent portable trust (DEC-2026-06-002). The positioning is "verified, driver-owned credentials, built on Midnight" — honest about the tech, never faking a per-fact proof before it runs.
