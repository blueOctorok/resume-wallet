...
- **Share modal (hub chrome + seamless social):** [`CareerCardShareModal.tsx`](src/components/hub/CareerCardShareModal.tsx) — **Share** tab uses [`HubSectionPanel`](src/components/hub/HubSectionPanel.tsx) + [`BlockCard`](src/components/ui/BlockCard.tsx) `variant="embed"` with teal / sky / violet / indigo vault accents (same pairing as candidate hub). Segmented **Share | Embed** tab strip, primary **lg** social CTA, **Apply & preview** grid (PDF / QR / Preview). **Embed** tab: three matching panels (iframe, email signature, README badge). **Seamless social:** `navigator.share()` or clipboard image + caption CTA. **Escape** on QR closes overlay first. **Block shell:** [`Modal`](src/components/ui/Modal.tsx) `panelShape="block"` + `ModalHeader` `variant="block"` — matches [`BlockPickerCategory`](src/components/hub/BlockPickerCategory.tsx) card chrome (`rounded-xl border`, dark `bg-gray-800/40`, light white) with subtle teal ring; modal body scroll and embed textareas use [`globals.css`](src/app/globals.css) **`.scrollbar-none`** (no visible bar, scroll still works).
