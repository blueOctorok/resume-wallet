...
--
-- career_cards view uses `SELECT mo.*` on mvr_orders, so Postgres
-- considers it dependent on every column. Must drop + recreate.

-- 1. Drop dependents (function → view)
...
-- 3. Recreate the view (identical to migration 064)
...
SELECT
  ...
  (SELECT count(*) FROM employment_verification_requests evr
   WHERE evr.driver_id = u.id
     AND evr.status IN ('VERIFIED', 'PARTIALLY_VERIFIED')) AS verified_jobs_count,
  CASE
...
COMMENT ON VIEW career_cards IS
  'Talent search + career card aggregates. verified_jobs_count = employment_verification_requests with status VERIFIED or PARTIALLY_VERIFIED (past employer responded on-file).';

-- 4. Recreate the search function (identical to migration 064)
...
COMMENT ON FUNCTION search_talent IS 'Search for candidates — uses career_cards view (065: same as 064, recreated after payment_tx_hash column widened).';
