-- Add self-ordered PSP signals to career_cards (FCRA: only ordered_by_company_id IS NULL).
-- Must drop search_talent + career_cards because the view column list changes.
...
SELECT
  ...
  (SELECT count(*) FROM psp_orders WHERE psp_orders.driver_user_id = u.id AND psp_orders.ordered_by_company_id IS NULL) AS psp_count,
  CASE
  ...
  CASE
    WHEN cdl.id IS NOT NULL THEN (
      ...
      (CASE WHEN psp.id IS NOT NULL THEN 20 ELSE 0 END) +
...
COMMENT ON VIEW career_cards IS
  'Talent search + career card aggregates. has_psp / psp_count = candidate-self PSP orders only (FCRA).';
...
COMMENT ON FUNCTION search_talent IS 'Search for candidates — uses career_cards view (072: adds has_psp for self-ordered PSP).';
