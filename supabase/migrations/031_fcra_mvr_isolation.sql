-- ============================================================
-- MIGRATION 031: FCRA MVR Isolation
-- ============================================================
-- Date: March 2026
--
-- Problem:
--   The career_cards view's has_mvr flag and latest_mvr_id pick
--   up ANY mvr_order for a driver, including employer-ordered ones.
--   This means:
--     - A driver can see on their own card that an employer ran
--       a check on them (CRA risk).
--     - Employer B can see an MVR that Employer A paid for.
--
-- Fix:
--   Scope the MVR LATERAL join to ONLY self-ordered MVRs
--   (ordered_by_company_id IS NULL). Employer-ordered MVRs are
--   fetched separately per-company at the API layer and never
--   written back to the shared career card view.
--
--   Canonical rule:
...
-- ── Drop view + dependent function ────────────────────────────────────────────
DROP VIEW IF EXISTS career_cards CASCADE;

-- ── Recreate career_cards view — MVR scoped to self-ordered only ───────────
...
SELECT
  ...
  -- FCRA isolation: only self-ordered MVRs appear in the shared view.
  -- Employer-ordered MVRs (ordered_by_company_id IS NOT NULL) are private
  -- to the ordering company and fetched separately at the API layer.
  ...
  -- Count only self-ordered MVRs in the public tally
  (SELECT COUNT(*) FROM mvr_orders
   WHERE driver_user_id = u.id
     AND ordered_by_company_id IS NULL) AS mvr_count,
  CASE
...
-- Resume LATERAL: only pick a resume whose source_role matches the candidate's role.
LEFT JOIN LATERAL (
...
-- FCRA isolation: self-ordered MVRs only. ordered_by_company_id IS NULL = driver paid.
LEFT JOIN LATERAL (
  SELECT * FROM mvr_orders mo
  WHERE mo.driver_user_id = u.id
    AND mo.ordered_by_company_id IS NULL
...
-- ── Recreate search_talent function ──────────────────────────────────────────
-- Drop any prior signatures first (parameter types may have changed).
...
COMMENT ON VIEW career_cards IS
  'Aggregated candidate view. Role derived from profile data. Resume filtered by source_role. '
  'MVR filtered to self-ordered only (FCRA isolation — employer-ordered MVRs are private).';
