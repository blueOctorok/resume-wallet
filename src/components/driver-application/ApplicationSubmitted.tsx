          ...
          >
            <div className={`flex-shrink-0 rounded-xl p-2.5 ${
              ...
              {resumeAutoCreateStatus === 'creating' && (
                <Loader2 className={`h-5 w-5 animate-spin ${isDarkTheme(theme) ? 'text-teal-400' : 'text-teal-600'}`} />
              )}
              {resumeAutoCreateStatus === 'created' && (
                <CheckCircle className='h-5 w-5 text-white' />
              )}
              {resumeAutoCreateStatus === 'failed' && (
                <AlertCircle className={`h-5 w-5 ${isDarkTheme(theme) ? 'text-red-400' : 'text-red-500'}`} />
              )}
            </div>
            <div>
      ...
      <div className='mb-8'>
        ...
        <div className='space-y-4'>
            ...
            >
              <h3
                className={`font-semibold mb-2 flex items-center gap-2 ${
                  isDarkTheme(theme) ? 'text-indigo-400' : 'text-indigo-700'
                }`}
              >
          ...
          {verificationStatus === 'verified' && (
            <div
              className={`p-4 rounded-xl border ${
                isDarkTheme(theme) ? 'bg-gray-700/50 border-gray-600' : 'bg-gray-50 border-gray-200'
              }`}
            >
              <h3
