...
export default function DotApplicationFlow({
  ...
  // Resume auto-creation state (shown when DOT app completes and no resume exists)
  ...
  // Many drivers don't have resumes - we create one from their DOT data
  // -------------------------------------------------------
  
  // Check if user has any resumes after DOT completion
  useEffect(() => {
    const checkForResumes = async () => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          // Show prompt if user has no resumes
          setHasNoResume(!resumes || resumes.length === 0)
        }
      } catch (err) {
  ...
  // Create a resume from the profile data (populated by DOT app)
  const handleCreateResume = useCallback(async () => {
    if (!walletAddress || isCreatingResume) return
    
    setIsCreatingResume(true)
    
    try {
      // Fetch the profile (which was just updated with DOT data)
      const profileResponse = await fetch('/api/driver/profile', {
        headers: { 'x-wallet-address': walletAddress },
      })
      ...
      // Build structured data matching ResumeBuilder's format
      const structuredData = {
        ...
        autoGenerated: true, // Flag so we know this was auto-created
        source: 'dot_application',
      }
      
      // Create the resume
      const resumeResponse = await fetch('/api/resumes/create', {
        method: 'POST',
        headers: {
      ...
      console.error('❌ [DOT] Failed to create resume:', err)
      // Don't show error to user - this is a bonus feature, not critical
    } finally {
      setIsCreatingResume(false)
    }
  }, [walletAddress, isCreatingResume])
