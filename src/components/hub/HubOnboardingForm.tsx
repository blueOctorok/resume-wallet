 ...
 *
 * It is a full-screen overlay, not a dismissible modal.
 * The hub is blocked until both fields are submitted.
 *
 * On success: overlay disappears and the block picker opens with AvA's
 * suggested categories already filtered, so the user lands on relevant blocks.
 */
export default function HubOnboardingForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await completeOnboarding(occupation.trim(), seekingReason.trim(), walletAddress)
      // Open the block picker — suggested categories from AvA are now in the store
      openPicker()
    } catch {
  ...
  return (
    // z-[90] sits above the hub (z-10) and navigation (z-50) but below critical system modals
