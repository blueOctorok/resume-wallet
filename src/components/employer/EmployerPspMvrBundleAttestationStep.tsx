...
export default function EmployerPspMvrBundleAttestationStep({
  ...
  const handleSubmit = async () => {
    ...
    if (usesDriverOwnedFlow && !driverOwnershipAcknowledged) {
      setError('Check the box confirming you understand you are ordering these reports for your Provven file.')
      return
    }
    ...
    <div className={`${cardClass} p-6 sm:p-8`}>
      ...
      <div className={`border-t pt-6 space-y-5 ${isDark ? 'border-gray-700' : 'border-gray-200'}`}>
        ...
        <p className={`text-sm ${textSecondary}`}>
          ...
          {submitBehavior === 'consent-bundle-only'
            ? 'so your employer can run MVR or PSP later without asking you again. It is encrypted in Provven’s database.'
            : usesDriverOwnedFlow
              ? 'so the vendor can match your identity when you order your MVR and PSP. It is encrypted in Provven’s database.'
