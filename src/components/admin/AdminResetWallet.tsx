...
export default function AdminResetWallet({
  ...
  const [sessionUserId, setSessionUserId] = useState(initialWalletAddress)
  ...
  useEffect(() => {
    setSessionUserId(initialWalletAddress)
  }, [initialWalletAddress])
  ...
  const handleReset = async () => {
    ...
    const address = sessionUserId.trim()
    if (!effectiveKey) {
    ...
    try {
      const response = await fetch('/api/admin/reset-wallet', {
        ...
        body: JSON.stringify({ sessionUserId: address }),
      })
  ...
  const handleClearFormData = () => {
    if (typeof window !== 'undefined') {
      const address = sessionUserId.trim() || initialWalletAddress
      if (!address) {
        setError('Please enter a wallet address.')
        return
      }
      ...
      <div className="space-y-4">
        ...
        <div className="flex gap-3">
          <button
            ...
            disabled={!sessionUserId.trim() && !initialWalletAddress}
        ...
        {result && (
          <div className="rounded-md border border-teal-300 bg-teal-50 px-3 py-2 text-sm text-teal-800">
            <p className="font-semibold">Wallet {result.sessionUserId}</p>
