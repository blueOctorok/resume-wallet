      ...
      >
        ...
        >
          <div className="flex items-center gap-3">
            <div className={`p-2 rounded-lg ${theme !== 'dark' ? 'bg-teal-700/10' : 'bg-teal-600/20'}`}>
              <FileText className={`h-6 w-6 ${theme !== 'dark' ? 'text-teal-800 dark:text-teal-300' : 'text-teal-600 dark:text-teal-400'}`} />
            </div>
            <h2
        ...
        {/* Tabs */}
        <div className={`border-b ${theme !== 'dark' ? 'border-gray-200' : 'border-gray-700'}`}>
          <div className="flex">
            <button
          ...
          <div className="p-6 space-y-6">
            {loading ? (
              <div className="text-center py-12">
                ...
                <p className={theme !== 'dark' ? 'text-gray-600' : 'text-gray-400'}>Loading MVR data...</p>
              </div>
            ) : error ? (
              <div className="text-center py-12">
                <AlertCircle className="h-12 w-12 mx-auto mb-4 text-red-500" />
                <p className={theme !== 'dark' ? 'text-red-600' : 'text-red-400'}>{error}</p>
              </div>
            ) : (
              <>
                {/* Summary Cards */}
                ...
                >
                  <div className="flex items-center gap-3 mb-3">
                    <div className={`p-2 rounded-lg ${theme !== 'dark' ? 'bg-blue-200' : 'bg-blue-900/50'}`}>
                      <DollarSign className="h-5 w-5 text-blue-400" />
                    </div>
                    <h3 className={`font-bold ${theme !== 'dark' ? 'text-gray-900' : 'text-blue-100'}`}>
                      Payments
                    </h3>
                  </div>
                  <p className={`text-3xl font-extrabold mb-1 ${theme !== 'dark' ? 'text-blue-700' : 'text-blue-300'}`}>
                    {data?.payments.length || 0}
                  </p>
                  <p className={`text-sm font-medium ${theme !== 'dark' ? 'text-blue-600' : 'text-blue-400/80'}`}>
                ...
                >
                  <div className="flex items-center gap-3 mb-3">
                    <div className={`p-2 rounded-lg ${theme !== 'dark' ? 'bg-emerald-200' : 'bg-emerald-900/50'}`}>
                      <FileText className="h-5 w-5 text-emerald-400" />
                    </div>
                    <h3 className={`font-bold ${theme !== 'dark' ? 'text-gray-900' : 'text-emerald-100'}`}>
                      Orders
                    </h3>
                  </div>
                  <p className={`text-3xl font-extrabold mb-1 ${theme !== 'dark' ? 'text-emerald-700' : 'text-emerald-300'}`}>
                    {data?.orders.length || 0}
                  </p>
                  <p className={`text-sm font-medium ${theme !== 'dark' ? 'text-emerald-600' : 'text-emerald-400/80'}`}>
                ...
                {orphanedPayments.length > 0 && (
                  ...
                  >
                    <div className="flex items-center gap-3 mb-3">
                      <div className={`p-2 rounded-lg ${theme !== 'dark' ? 'bg-amber-200' : 'bg-amber-900/50'}`}>
                        <AlertCircle className="h-5 w-5 text-amber-400" />
                      </div>
                      <h3 className={`font-bold ${theme !== 'dark' ? 'text-gray-900' : 'text-amber-100'}`}>
                        Action Needed
                      </h3>
                    </div>
                    <p className={`text-3xl font-extrabold mb-1 ${theme !== 'dark' ? 'text-amber-700' : 'text-amber-300'}`}>
                      {orphanedPayments.length}
                    </p>
                    <p className={`text-sm font-medium ${theme !== 'dark' ? 'text-amber-600' : 'text-amber-400/80'}`}>
                      Pending completion
                    </p>
                  </div>
                )}
              ...
              {orphanedPayments.length > 0 && (
                ...
                >
                  <div className="flex items-start gap-4">
                    <div className={`p-3 rounded-xl ${theme !== 'dark' ? 'bg-amber-200' : 'bg-amber-900/60'}`}>
                      <AlertCircle className="h-6 w-6 text-amber-400" />
                    </div>
                    <div className="flex-1">
                      <h4 className={`font-bold text-lg mb-2 ${theme !== 'dark' ? 'text-gray-900' : 'text-amber-100'}`}>
                        {orphanedPayments.length} Payment{orphanedPayments.length > 1 ? 's' : ''} Awaiting Order Completion
                      </h4>
                      <p className={`text-sm mb-4 ${theme !== 'dark' ? 'text-gray-700' : 'text-amber-200/80'}`}>
                  ...
                  <>
                    ...
                    {data && data.payments.length > 0 ? (
                      ...
                      >
                        ...
                        >
                          <div className={`p-2 rounded-lg ${theme !== 'dark' ? 'bg-blue-100' : 'bg-blue-900/50'}`}>
                            <DollarSign className="h-5 w-5 text-blue-400" />
                          </div>
                          Payment History
                        </h3>
                              ...
                              >
                                <div className="flex justify-between items-start gap-2 mb-3">
                                  <div className="flex items-center gap-2 flex-1 min-w-0">
                                    <span className={`text-sm sm:text-base font-bold ${theme !== 'dark' ? 'text-gray-900' : 'text-white'}`}>
                                      ${payment.amount} USDC
                                    </span>
                                    {isOrphaned && (
                                ...
                                <div className={`text-xs space-y-1.5 ${theme !== 'dark' ? 'text-gray-600' : 'text-gray-400'}`}>
                                  <div className="flex items-center gap-1.5">
                                    <span className="font-medium">Tx:</span>
                                    <code className={`px-1.5 py-0.5 rounded font-mono text-[10px] truncate ${
                                      theme !== 'dark' ? 'bg-gray-200' : 'bg-gray-900/50 border border-gray-700'
                                    }`}>
                      ...
                      >
                        <div className={`p-4 rounded-full mx-auto mb-4 w-fit ${
                          ...
                          <DollarSign className={`h-16 w-16 ${theme !== 'dark' ? 'text-gray-400' : 'text-gray-600'}`} />
                        </div>
                        <p className={`text-xl font-bold mb-2 ${theme !== 'dark' ? 'text-gray-900' : 'text-white'}`}>
                          No Transactions Yet
                        </p>
                ...
                >
                  ...
                  >
                    <div className={`p-2 rounded-lg ${theme !== 'dark' ? 'bg-emerald-100' : 'bg-emerald-900/50'}`}>
                      <FileText className="h-5 w-5 text-emerald-400" />
                    </div>
                    Order History
                  </h3>
                  <div className="space-y-3">
                      ...
                      >
                        <div className="flex justify-between items-start gap-2 mb-3">
                          <div className="flex-1 min-w-0 pr-2">
                            <span className={`text-[10px] sm:text-xs font-mono font-medium truncate block ${theme !== 'dark' ? 'text-gray-900' : 'text-white'}`}>
                        ...
                        <div className={`text-sm mb-4 flex items-center gap-2 ${theme !== 'dark' ? 'text-gray-600' : 'text-gray-400'}`}>
                    ...
                    >
                      <div className={`p-4 rounded-full mx-auto mb-4 w-fit ${
                        ...
                        <FileText className={`h-16 w-16 ${theme !== 'dark' ? 'text-gray-400' : 'text-gray-600'}`} />
                      </div>
                      <p className={`text-xl font-bold mb-2 ${theme !== 'dark' ? 'text-gray-900' : 'text-white'}`}>
                        No MVR Orders Yet
                      </p>
                      <p className={`text-sm mb-6 ${theme !== 'dark' ? 'text-gray-600' : 'text-gray-400'}`}>
