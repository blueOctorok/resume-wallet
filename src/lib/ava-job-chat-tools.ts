...
export const STORMI_JOB_CHAT_TOOLS = [
  ...
  },
  {
    name: 'save_job_alert',
    description:
      'Create a daily email-style in-app job alert: Provven will scan for new listings on a schedule and notify when the assistant scores a strong match. Use when the user wants ongoing monitoring, alerts, or to "watch" a search. Respect limits (2 alerts free, 5 with AI credits). At most one save per user message unless they explicitly ask for two different alerts.',
    input_schema: {
      type: 'object' as const,
      properties: {
