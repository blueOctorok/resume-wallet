...
export async function buildCareerCardPdfBuffer(
  ...
  pdf.setFontSize(10)
  const headline = card.occupation || 'Provven Career Card'
  pdf.text(headline.slice(0, 90), margin, 30)
  pdf.setFontSize(8)
  pdf.setTextColor(...TEAL)
  pdf.text('Verified by Provven — selective disclosure career platform', margin, 37)
  ...
  pdf.setFontSize(8)
  pdf.setTextColor(...MUTED)
  pdf.text('Scan for live Career Card on Provven', margin + 36, y + 10)
  pdf.text(publicCardUrl.slice(0, 80), margin + 36, y + 16)
  pdf.setFont('helvetica', 'bold')
  pdf.setTextColor(...TEAL)
  pdf.text('provven.com', margin, pageH - 8)
