 ...
 * Example: `/api/oembed?url=https://stormchain.ai/card/TOKEN`
 */
export async function GET(request: NextRequest) {
  try {
    ...
    if (!match?.[1]) {
      return NextResponse.json({ error: 'URL must be a Storm career card link (/card/{token})' }, { status: 404 })
    }
    ...
    const iframe = `<iframe src="${embedUrl}" width="420" height="360" style="border:0;border-radius:12px;max-width:100%;" title="${escapeAttr(card.name)} — Storm Career Card" loading="lazy" referrerpolicy="strict-origin-when-cross-origin"></iframe>`
