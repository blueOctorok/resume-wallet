...
export async function POST(request: Request) {
  try {
    ...
    if (!userToUpdate) {
      console.log(`[SET ROLE] User not found for wallet ${walletAddress}, creating new user...`);
      ...
      console.log(`[SET ROLE] New user created with ID: ${userToUpdate.id}, role: ${role}`);
    }
    ...
    console.log(`[SET ROLE] User ${userToUpdate.id} changing role from "${userToUpdate.role}" to "${newRole || 'NULL (cleared)'}"`);
    ...
    if (newRole === 'employer') {
      ...
      if (!existingCompany) {
        ...
        if (companyError) {
          ...
          console.log(`[SET ROLE] Company record created for employer user ${userToUpdate.id}`);
        }
      }
    }
