  ...
  try {
    ...
    if (await companyHasScreeningConsentBlock(supabase, companyId)) {
      return NextResponse.json(
        {
          error:
            'Your company collects screening consent in Provven first. After the candidate finishes the three-step package, place PSP orders with POST /api/employer/screenings/order (payment + consent bundle id).',
        },
        { status: 400 },
      )
    }
    ...
    if (!(await companyCanOrderPsp(supabase, companyId))) {
      return NextResponse.json(
        {
          error:
            'PSP ordering is not enabled for your company. A company owner or admin must install the PSP ordering block (or contact Provven support).',
        },
        { status: 403 },
      )
    }
